...
* 通用操作
  ...
  7. in用来检查指定元素是否存在于列表中，如果存在，返回True，否则返回False

  8. not in用来检查指定元素是否不在列表中，如果不在，返回True，否则返回False
  ...
  12. s.index("元素") 获取指定元素在列表中的第一次出现时索引。第二个参数，表示查找的起始位置 ， 第三个参数，表示查找的结束位置，第二第三可选

  13. s.count() 统计指定元素在列表中出现的次数

  14. append() 向列表的最后添加一个元素

  15. insert() 向列表的指定位置插入一个元素 stus.insert(2,'唐僧')

  16. extend()  需要一个序列作为参数，它会将该序列中的元素添加到当前列表中 stus.extend(['唐僧','白骨精'])

  17. clear() 清空序列

  18. pop() 根据索引删除并返回被删除的元素 stus.pop(2)  无参数删除最后一个

  19. remove() 删除指定值得元素，如果相同值得元素有多个，只会删除第一个 stus.remove('猪八戒')

  20. reverse() 用来反转列表  stus.reverse()

  21. sort() 用来对列表中的元素进行排序，默认是升序排列，如果需要降序排列，则需要传递一个reverse=True作为参数

  22. range()是一个函数，可以用来生成一个自然数的序列
...
1. 类的定义
   ...
   ![image-20211119195340168](../../BaiduNetdiskWorkspace/typora/images/image-20211119195340168.png)
...
![image-20211121095322581](../../BaiduNetdiskWorkspace/typora/images/image-20211121095322581.png)
